...
class Chinchilla:
    ...
    def __init__(
        ...
        # loss_fn: Callable = log_huber,  # Fits to the floor (\approx. lower bound) of the distribution $L(N, D)$
        ...
        self.database = Database(self.project_dir, log_level=log_level, csv_file=csv_file, df=df)
    ...
    def fit(self, parallel: bool = True, simulation: bool = False) -> None:
        ...
        self._const = dict(
            ...
            y_true=_df[self.loss_name].values.astype(DTYPE),
            weights=weights,
        )
        ...
        self.visualizer.LBFGS(y_pred, _df[self.loss_name].values, simulation=simulation)
    ...
    def report(self, plot: bool = True) -> None:
        ...
        if len(self.database.df):
            ...
            y_true = self.database.df[self.loss_name].values
